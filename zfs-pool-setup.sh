#!/usr/bin/env bash
# Create a ZFS pool with raidz3, raidz2, or dRAID data vdevs, optional mirrored log (SLOG), L2ARC cache,
...
#   DRAID_VDEV_SPEC=draid3:9d:26c:2s zfs-draid.sh SEAGATE 78
#   SPECIAL=Y DRY_RUN=1 zfs-pool-setup.sh --raidz3 SEAGATE 78
#   CACHE=N zfs-pool-setup.sh --raidz3 --special=mirror3x3+1spare SEAGATE 78
#   zfs-pool-setup.sh --draid --special=mirror10 SEAGATE 78
...
#                    (a script not named zfs-draid* / zfs-pool-setup uses its basename). Create/dry-run always
#                    asks you to verify the name; SKIP_CONFIRM=1 / non-tty accept the default.
#   DATA_DISK_SOURCE  mpath | nvme (default: mpath)
#   RAID_TOPOLOGY    draid | raidz2 | raidz3 (default: draid). CLI: --draid / --raidz2 / --raidz3 / --topology=.
#                    Assessment lists all three; raidz3 and raidz2 with a special vdev are ranked first.
#   DRAID_PARITY     2 or 3 (default: 3). dRAID layout (D,S) is computed per vdev child count. Ignored for raidz*.
...
#   DISKS_PER_VDEV   Disks per data vdev (draid default: 26; raidz2/raidz3 default: 13; nvme default: total count)
...
DRAID_MAX_WIDTH="${DRAID_MAX_WIDTH:-40}"   # widest dRAID vdev the assessment proposes / sanity limit
RAIDZ_MAX_WIDTH="${RAIDZ_MAX_WIDTH:-15}"   # widest raidz3 vdev (raidz2 is capped at 13)
DRY_RUN="${DRY_RUN:-0}"
...
if [[ -n "${POOL:-}" ]]; then
  ...
  if [[ "$_pool_from_fn" == zfs-draid* || "$_pool_from_fn" == zfs-pool-setup ]]; then
    default_pool_name_from_host
  else
    POOL="$_pool_from_fn"
...
usage() {
  ...
  echo "      --raidz3            Same as --topology=raidz3 (classic parity; default ${DEFAULT_TOTAL_DISKS} → 6×13)"
  ...
  echo "  total_hdd_count      Data disks for the vdev(s) (default: ${DEFAULT_TOTAL_DISKS} for mpath;"
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --draid)
        ...
        ;;
      --raidz2)
        RAID_TOPOLOGY=raidz2
        shift
        ;;
      --raidz3)
        RAID_TOPOLOGY=raidz3
        shift
        ;;
      --topology)
        if [[ -z "${2:-}" || "$2" == --* ]]; then
          echo "Error: --topology requires draid, raidz2, or raidz3." >&2
          usage 1
        fi
        RAID_TOPOLOGY="$2"
        shift 2
...
# --- Data-vdev topology table ---
# Every topology-specific fact lives here. To add a topology (e.g. "mirror"): extend SUPPORTED_TOPOLOGIES,
# the normaliser, and each accessor below; the create path and the assessment call only these accessors.
SUPPORTED_TOPOLOGIES="draid raidz2 raidz3"

topology_normalize() { # $1 user value → canonical name, or return 1
  case "${1,,}" in
    draid | draid2 | draid3) echo draid ;;
    raidz2 | z2) echo raidz2 ;;
    raidz3 | z3) echo raidz3 ;;
    *) return 1 ;;
  esac
}
topology_parity() { # disks of redundancy per data vdev
  case "$1" in draid) echo "$DRAID_PARITY" ;; raidz2) echo 2 ;; raidz3) echo 3 ;; esac
}
topology_default_width() { # ORCD house width per data vdev
  case "$1" in draid) echo 26 ;; raidz2 | raidz3) echo 13 ;; esac
}
topology_min_width() { # smallest width zpool accepts / that makes sense
  case "$1" in draid) echo $(( DRAID_PARITY + 2 )) ;; raidz2) echo 4 ;; raidz3) echo 5 ;; esac
}
topology_max_sane_width() { # wider than this is refused (stripe too wide for HPC small-file workloads)
  case "$1" in draid) echo "$DRAID_MAX_WIDTH" ;; raidz2 | raidz3) echo "$RAIDZ_MAX_WIDTH" ;; esac
}
topology_vdev_spec() { # $1 topology, $2 width → the zpool vdev keyword for one data vdev
  case "$1" in
    draid)
      if [[ -n "${DRAID_VDEV_SPEC:-}" ]]; then echo "$DRAID_VDEV_SPEC"; else compute_best_draid_spec "$2" "$DRAID_PARITY" "$DRAID_PROFILE"; fi
      ;;
    raidz2 | raidz3) echo "$1" ;;
  esac
}
topology_label() { # human label for one data vdev, e.g. "raidz3 of 13" / "draid3:9d:26c:2s"
  local spec
  spec=$(topology_vdev_spec "$1" "$2" 2>/dev/null || echo "$1")
  case "$1" in draid) echo "$spec" ;; *) echo "${spec} of $2" ;; esac
}

RAID_TOPOLOGY="${RAID_TOPOLOGY:-draid}"
if ! _t=$(topology_normalize "$RAID_TOPOLOGY"); then
  echo "Error: RAID_TOPOLOGY / --topology must be one of: ${SUPPORTED_TOPOLOGIES} (got '${RAID_TOPOLOGY}')." >&2
  exit 1
fi
RAID_TOPOLOGY=$_t
unset _t
DATA_PARITY=$(topology_parity "$RAID_TOPOLOGY")
...
if [[ "$ASSESS_ONLY" == "1" ]] || [[ $# -lt 1 ]]; then
  ...
  if (( SPECIAL_ENABLED )) && [[ "$DATA_DISK_SOURCE" == nvme ]]; then
    echo "Error: special vdev is not supported with DATA_DISK_SOURCE=nvme (local NVMe are used for data vdevs)." >&2
    exit 1
  fi
  ...
  DISKS_PER_VDEV="${DISKS_PER_VDEV:-$(topology_default_width "$RAID_TOPOLOGY")}"
  ...
  _minw=$(topology_min_width "$RAID_TOPOLOGY")
  if (( DISKS_PER_VDEV < _minw )); then
    echo "Error: ${RAID_TOPOLOGY} needs at least ${_minw} disks per vdev (DISKS_PER_VDEV=${DISKS_PER_VDEV})." >&2
    exit 1
  fi
  _maxw=$(topology_max_sane_width "$RAID_TOPOLOGY")
  if (( DISKS_PER_VDEV > _maxw )) && [[ -z "${DRAID_VDEV_SPEC:-}" ]]; then
    echo "Warning: DISKS_PER_VDEV=${DISKS_PER_VDEV} is wider than the ${RAID_TOPOLOGY} sanity limit (${_maxw}); wide stripes hurt small-file efficiency and rebuild time." >&2
  fi
  unset _minw _maxw
...
print_visual_pool_layout() {
  ...
  echo "Data topology:     ${RAID_TOPOLOGY}  (${NUM_VDEVS} × ${ONE_DATA_LABEL})"
  echo "Total data disks:  $TOTAL_DISKS"
  echo
  for ((v = 0; v < ${#VDEV_DISKS_ARRAYS[@]}; v++)); do
    # shellcheck disable=SC2206
    paths=(${VDEV_DISKS_ARRAYS[$v]})
    echo "--- ${ONE_DATA_LABEL} vdev $((v + 1)) (${#paths[@]} disks) ---"
    n=0
    for d in "${paths[@]}"; do
      ((++n))
...
write_pool_setup_log_preamble() {
  ...
  shift
  {
    ...
    if [[ "$RAID_TOPOLOGY" == draid ]]; then
      echo "topology: ${RAID_TOPOLOGY}  ${ONE_DATA_SPEC} (parity=${DATA_PARITY}, profile=${DRAID_PROFILE})"
    else
      echo "topology: ${RAID_TOPOLOGY}  ${ONE_DATA_SPEC} (parity=${DATA_PARITY}, width=${DISKS_PER_VDEV})"
    fi
    if (( SPECIAL_ENABLED )); then
      echo "special:  ${SPECIAL_LAYOUT} ($(special_layout_summary "$SPECIAL_LAYOUT"))"
    else
...
prompt_confirm_zpool_create() {
  ...
  local r
  echo "About to create pool '${POOL}': ${NUM_VDEVS} × ${ONE_DATA_LABEL} (${TOTAL_DISKS} data disks)${SPECIAL_LAYOUT:+, special ${SPECIAL_LAYOUT}}."
...
# Pick raidz2/raidz3 vdev width for N unused disks. Prefer the ORCD 13-wide layout when it divides
# (78 → 6×13, 104 → 8×13). Otherwise maximize usable data disks (width − parity) with ≥2 vdevs,
# leftover disks become pool hot spares. raidz2 widths 6–13; raidz3 widths 7–15.
assess_pick_raidz_width() {
  local total=$1 parity=$2
  local w minw maxw nv leftover data dd i max_data=0 best="" best_sec=999999
  local -a cand_w=() cand_data=() cand_sec=()
  minw=$((parity + 4))
  maxw=$RAIDZ_MAX_WIDTH
  (( parity == 2 && maxw > 13 )) && maxw=13
  (( maxw > total )) && maxw=$total
  for ((w = minw; w <= maxw; w++)); do
    ...
    data=$((nv * (w - parity)))
    dd=$(( w > 13 ? w - 13 : 13 - w ))
    cand_w+=("$w"); cand_data+=("$data")
    # secondary: the ORCD 13-wide layout first, then few leftover disks (they become hot spares)
    cand_sec+=("$(( dd * 10 + leftover * 5 ))")
    (( data > max_data )) && max_data=$data
  done
  if ((${#cand_w[@]} == 0)); then
    (( total >= minw )) && echo "$total" || echo "$minw"
    return
  fi
  # Any width within 10% of the best usable capacity is acceptable; 106 disks → 8×13 (+2 spares), not 7×15.
  for i in "${!cand_w[@]}"; do
    (( cand_data[i] * 100 >= max_data * 90 )) || continue
    if (( cand_sec[i] < best_sec )); then
      best=${cand_w[i]}
...
assess_print_create_option() {
  ...
  local nvdev usable spec_u ratio cmd extra topology def_width topo_flag=""
  topology="${ASSESS_TOPOLOGY:-draid}"
  nvdev=$((ndisks / width))
  if [[ "$spec" == draid* ]]; then
    usable=$(assess_draid_usable_bytes "$spec" "$disk_b" "$nvdev")
  else
    usable=$(( (width - parity) * nvdev * disk_b ))
  fi
  ((++ASSESS_RANK))
  ...
  if [[ "$spec" == draid* ]]; then
    ...
    echo "    Data:     ${ndisks}× $(human_bytes "$disk_b") ${vendor}  →  ${nvdev} × ${spec} of ${width}  (${parity} parity, $((width - parity)) data per vdev)"
  fi
  if (( hdd_spares > 0 )); then
  ...
  if [[ -n "$layout" ]]; then
    ...
    cmd="./$(basename "$SCRIPT_PATH") ${topo_flag}${vendor} ${ndisks}"
  fi
  echo "$extra"
  echo "    Usable:   ~$(human_bytes "$usable") data"
  local env="POOL=${POOL}"
  def_width=$(topology_default_width "$topology")
  [[ "$topology" == draid && "$parity" != 3 ]] && env+=" DRAID_PARITY=${parity}"
  [[ "$width" != "$def_width" ]] && env+=" DISKS_PER_VDEV=${width}"
...
run_storage_assessment() {
...

  {
    ...
    echo
    echo "Priority: raidz3, then raidz2, then dRAID — each WITH a special vdev (mirrored NVMe"
    echo "for metadata / small blocks) when unused NVMe exist. Without a special vdev, metadata"
    echo "lives on HDD and directory-heavy workloads suffer. SLOG + L2ARC use the smallest NVMe."
    echo
    ...
    for key in ${hdd_unused_keys[@]+"${hdd_unused_keys[@]}"}; do
      ...
      if [[ -n "$best_layout" ]]; then
        ...
        assess_print_raidz_recommended 2 "$navail" "$vendor" "$disk_b" "${large_sz:-0}"
        echo "--- dRAID (distributed parity; sequential rebuild) ---"
      fi
      ASSESS_TOPOLOGY=draid
    ...
    if (( special_fit == 0 )); then
      for key in ${hdd_unused_keys[@]+"${hdd_unused_keys[@]}"}; do
        ...
        disk_b=${grp_size[$key]}
        navail=$ndisks
        rz_w=$(assess_pick_raidz_width "$navail" 3)
        rz_n=$((navail - navail % rz_w))
        if (( rz_n >= rz_w && rz_w >= 7 )); then
          ASSESS_TOPOLOGY=raidz3
          assess_print_create_option \
            "LAST RESORT — raidz3 WITHOUT special vdev (metadata on HDD)" \
            3 "$rz_w" "$rz_n" "$vendor" "" "$disk_b" "raidz3" 0 "$((navail - rz_n))"
        fi
        ASSESS_TOPOLOGY=draid
        width=$(assess_pick_width "$navail")
        ndisks=$((navail - navail % width))
        (( ndisks >= width && width >= 8 )) || continue
        spec=$(compute_best_draid_spec "$width" 3 balanced 2>/dev/null || true)
        [[ -n "$spec" ]] || continue
    ...
    if (( ASSESS_RANK == 0 )); then
      echo "No deployable raidz/dRAID combination found."
...
ONE_DATA_SPEC=$(topology_vdev_spec "$RAID_TOPOLOGY" "$DISKS_PER_VDEV") || exit 1
ONE_DATA_LABEL=$(topology_label "$RAID_TOPOLOGY" "$DISKS_PER_VDEV")
...
# unless -f is given. raidz3/dRAID3 data + 2-way mirrored NVMe special is the intended design, so add -f.
if (( SPECIAL_ENABLED )); then
  _sp_red=$(special_layout_redundancy "$SPECIAL_LAYOUT")
  if (( _sp_red < DATA_PARITY )); then
    _force_reason="${_force_reason:+${_force_reason}; }special ${SPECIAL_LAYOUT} redundancy ${_sp_red} < ${RAID_TOPOLOGY} parity ${DATA_PARITY} (mismatched replication level)"
  fi
  unset _sp_red
fi
...
for local_vdev_idx in "${!VDEV_DISKS_ARRAYS[@]}"; do
  ...
  _zpool_cmd+=("$ONE_DATA_SPEC" "${local_vdev_names[@]}")
done
...
_desc="${NUM_VDEVS} × ${ONE_DATA_LABEL}"
